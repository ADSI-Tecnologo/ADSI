...
var listado = new Array(1,2,3,4,5,6,7,8);
...
var verificar_respuesta = function() {
  ...
  if( !$('#'+PREG).hasClass('verificada') ) {
    ...
    //$('#retroalimentacion').fadeIn();
    if(dataRESP) {
      $(this).addClass('buena');
      aciertos.push(pos);
      $('.c'+pos).addClass('gano');
      $('#personaje').removeClass('normal').removeClass('perdio').addClass('gano');
      if(conaudio<2) $('audio')[1].play();
      //$('#retroalimentacion p').html(rb[preguntas[pos]]);

    } else {
      $(this).addClass('mala');
      errores.push(pos);
      $('.c'+pos).addClass('perdio');
      $('#personaje').removeClass('normal').removeClass('gano').addClass('perdio');
      if(conaudio<2) $('audio')[2].play();
      //$('#retroalimentacion p').html(rm[preguntas[pos]]);

    }
    $('.c'+pos).removeClass('activo');

    $('.respuesta').unbind('click');
    setTimeout(siguiente_pregunta,1500);
...
function siguiente_pregunta() {
  ...
  $('.icon_respuesta--buena').hide();
  $('.icon_respuesta--mala').hide();
  ...
  $('#p'+preguntas[pos]+' .question p span').html((pos+1) + '. ');
  $('.c'+pos).addClass('activo');

  $('#retroalimentacion').hide();

  $('#personaje').removeClass('perdio').removeClass('gano').addClass('normal');
